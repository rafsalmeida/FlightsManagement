                                    ...
                                    <p class="description">
                                        @foreach($movimentosAeronave as $key => $movimento)
                                            @if(isset($datas[$i]) && $movimento==$datas[$i])
                                                Movimento {{$key}}<form method="GET" action="{{action('MovimentoController@edit', $key)}}" role="form" class="inline">
                                        @csrf
                                        @method('get')
                                        <input type="hidden" name="movimento_id" value="{{ $key }}">
                                        {!! Form::button('<i class="fas fa-exclamation-triangle"></i> Editar', ['type' => 'submit', 'class' => 'btn btn-primary', 'style'=>"width:100%"]) !!}
                                    </form>
                                    <br>
                                    @endif
                                    @endforeach
